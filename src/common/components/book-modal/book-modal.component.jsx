        ...
        <DialogContent>
          <div className="flex flex-col items-center gap-6">
            ...
            <div className="mt-[14px] flex gap-5">
              <CustomButton
                className="items-center justify-center rounded-md border-[1.5px] border-solid border-primary px-[30px] py-[7px] text-base font-bold not-italic leading-[26px] text-primary"
                text="Save as draft"
                onClick={handleSave}
              />
              <CustomButton
                className="btn-primary items-center px-[30px] py-[7px] text-sm font-semibold not-italic leading-[normal]"
                text={!isLoadingButtonBook && "Book"}
                onClick={handleBook}
                {...(isLoadingButtonBook && {
                  startIcon: <Loader loading={isLoadingButtonBook} />,
                })}
                // startIcon={<Loader loading={isLoadingButtonBook} />}
                disabled={isLoadingButtonBook}
              />
            </div>
          </div>
        </DialogContent>
