...
export default function useCompletedCampaign() {
  ...
  const completedCampaigns =
    applications?.filter((app) => app.status === "HIRED" && app.campaign?.status === "COMPLETED") ||
    [];
  ...
  const formattedCampaigns = filteredCampaigns.map((campaign) => formatCampaignData(campaign));

  // Auto-select first campaign when campaigns are loaded
  useEffect(() => {
    if (formattedCampaigns.length > 0) {
      // Always select the first campaign when campaigns are available
