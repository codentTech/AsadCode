...
function Features({ isCreatorMode }) {
  const features = [
    ...
    },
    {
      ...
      title: isCreatorMode ? "Finance Dashboard" : "Precision Discovery",
      description: isCreatorMode
        ? "See your full financial picture — including budget, escrow status, and campaign spend across time."
            ...
            <React.Fragment key={idx}>
              {feature.title && (
                <div className="group bg-white rounded-xl p-6 shadow-md hover:shadow-xl transition-all duration-300 transform hover:-translate-y-0 border border-gray-100">
                  ...
                  <p className="text-gray-600 ml-18 pl-1">
                    {feature.description}
                  </p>
                </div>
              )}
