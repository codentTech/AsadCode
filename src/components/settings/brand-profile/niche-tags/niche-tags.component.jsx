...
const NicheTags = () => {
  ...
  // Load existing niche tags from user profile
  useEffect(() => {
    const user = getUser();
    if (user?.brand_profile?.target_niches) {
      setSelectedTags(user.brand_profile.target_niches);
    }
  }, []);
  ...
  const handleNicheSelect = (niche) => {
    ...
    if (!selectedTags.includes(niche)) {
      setSelectedTags([...selectedTags, niche]);
    }
    setSearchTerm("");
  ...
  const handleNicheRemove = (niche) => {
    setSelectedTags(selectedTags.filter((tag) => tag !== niche));
  };
  ...
  const handleKeyPress = (e) => {
    if (e.key === "Enter" && filteredNiches.length > 0 && selectedTags.length < 5) {
      e.preventDefault();
  ...
  const onSubmit = async () => {
    ...
    const payload = {
      filming_preference: user.brand_profile?.filming_preference || "remote",
      campaign_types: user.brand_profile?.campaign_types || [],
      target_niches: selectedTags,
      creator_sizes: user.brand_profile?.creator_sizes || [],
      geographic_focus: user.brand_profile?.geographic_focus || [],
    };

    const result = await dispatch(setupBrandCampaignPreferences({ payload, email: user.email }));
      ...
      <div className="bg-white/80 backdrop-blur-sm rounded-xl shadow-lg border border-white/20 p-6">
        ...
        <div className="mb-6">
          <div className="relative">
            <CustomInput
              label="Search and select niches"
              customRef={inputRef}
              name="nicheSearch"
              value={searchTerm}
              ...
              placeholder={
                selectedTags.length >= 5
                  ? "Maximum 5 niches selected. Remove one to add another."
                  : "Type to search all available niches..."
              }
            ...
            {showSuggestions && filteredNiches.length > 0 && selectedTags.length < 5 && (
            ...
            {showSuggestions && filteredNiches.length === 0 && searchTerm.trim() !== "" && (
              <div className="absolute z-10 w-full mt-1 bg-white border border-gray-200 rounded-lg shadow-lg p-4">
                <p className="text-sm text-gray-500 text-center">
                  No niches found for "{searchTerm}"
                </p>
              </div>
            )}
          </div>
          ...
          <div className="mb-6">
            ...
            <div className="flex flex-wrap gap-2">
              {selectedTags.map((tag) => (
                <span
                  key={tag}
                  className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-primary text-white rounded-full text-xs font-medium"
                >
                  {tag}
                  <button
                    type="button"
                    onClick={() => handleNicheRemove(tag)}
